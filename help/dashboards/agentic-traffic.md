...
LLM Optimizerは、CDN ログからフィールドのサブセットのみを取得して処理します。 生のログフィールド名は CDN プロバイダーによって異なりますが、正規化されて次のように表示されます。
...
上位および下位の Movers ビューでは、エージェンティックトラフィックの週別の変化が最も大きい URL がハイライト表示されます。つまり、コンテンツにアクセスする AI システムからの訪問やヒットです。 **トップムーバー** は、表示やエンゲージメントを向上させるページを表示し、**ボトムムーバー** は、最も急激に低下した URL を表示します。 これにより、どのコンテンツが上昇傾向にあり、注意が必要か、AI 駆動の検出パターンが変化しているかをすばやく特定できます。
