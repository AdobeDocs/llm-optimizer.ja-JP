...
これらの正規化されたフィールドは、Agentic View を通じて公開されます。 [ リファラルトラフィック ](/help/dashboards/referral-traffic.md) ダッシュボードでは、CDN ログを使用してページヒット指標を表示します。 個人を特定できる情報（PII）は、CDN ログの取り込みまたはその後のデータ処理のどの段階でも処理または保存されません。
...
URL の詳細ビューを使用すると、ページのパフォーマンスの全体像を把握でき、ページの引用頻度、ページが言及される AI 応答のセンチメント、ページが表示されるトピックとプロンプト、エージェンティックトラフィックとリファラルトラフィックのトレンドを示すことができます。
