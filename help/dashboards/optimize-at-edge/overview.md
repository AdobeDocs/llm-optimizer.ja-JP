...
### ステージング ドメイン API キー（オプション）

本番稼働前にステージングホスト名でテストを行う場合は、LLM Optimizerを使用して&#x200B;**one** ステージングドメインを登録し、**Customer configuration** → **CDN configuration** → **最適化をAI エージェントにデプロイ** → **ステージングドメイン** （または&#x200B;**ステージドメイン**）から&#x200B;**ステージング** API キーをコピーします。 ステージング ホスト名は、実稼動サイトと同じ登録可能ドメインを共有する必要があります。 **独自のCDNを持ち込む** ガイドには、ステージング キーを取得し、ステージング URLでルーティングを検証するための完全な手順が含まれています。
